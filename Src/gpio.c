...
void __gpio_init(void) __attribute__((constructor));
...
// this adds 0x400 for GPIOB, 0x800 for GPIOC, ...
#define PORT(Pxy)   (GPIO_TypeDef*)(GPIOA_BASE + ((Pxy & 0xF0) << 6))
#define PIN(Pxy)    (Pxy & 0x0F)


// configure pin Pxy as a digital input
void gpio_input(uint8_t Pxy) {
    GPIO_TypeDef *port = PORT(Pxy);
    int pin = PIN(Pxy);
...
// configure pin as a digital output (open-drain)
